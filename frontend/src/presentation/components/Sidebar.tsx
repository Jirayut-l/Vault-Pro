...
export function Sidebar() {
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-slate-950 text-slate-200 border-r border-slate-900 flex flex-col transition-all duration-300 ${
      ...
      <div className="h-16 px-6 border-b border-slate-900 flex items-center justify-between">
        <Link href="/dashboard" className="flex items-center space-x-2">
          ...
          <span className="font-bold text-lg tracking-wider bg-gradient-to-r from-primary to-sky-400 bg-clip-text text-transparent">
            VAULT PRO
          </span>
        </Link>
        <button
          onClick={() => setSidebarOpen(false)}
          className="p-1 rounded-md text-slate-400 hover:text-slate-200 hover:bg-slate-900 transition-colors md:hidden"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
